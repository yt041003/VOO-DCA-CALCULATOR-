...
The repository deliberately does **not** ship invented VOO history. With no market-data configuration, the API route returns an explicitly labeled synthetic development dataset so the interface and engine can be evaluated. The site emits `noindex`/robots blocking until `MARKET_DATA_API_URL` is configured, and synthetic results must never be presented as historical performance.
...
Set `NEXT_PUBLIC_SITE_URL` to the production origin for canonical and sitemap URLs. On Vercel, the app otherwise uses `VERCEL_PROJECT_PRODUCTION_URL` automatically.

## Calculation assumptions

Contributions are bought on the first available trading day in each calendar month. The initial investment is added to the first purchase. Fractional shares are supported. Dividends are processed on the first price date on or after the dividend event; DRIP buys shares at that raw close, while non-DRIP distributions remain cash. Taxes, fees, slippage, and spreads are excluded. The annualized display is a money-weighted XIRR based on dated user contributions and the ending portfolio value.

## Vercel launch checklist

1. Set `NEXT_PUBLIC_SITE_URL` to the stable production URL (not a per-deployment preview URL).
2. Configure `MARKET_DATA_API_URL` and, when required, `MARKET_DATA_API_KEY`.
3. Redeploy, then verify `/api/market-data` reports the real provider rather than `Synthetic demonstration data`.
4. Confirm `/robots.txt`, `/sitemap.xml`, and the calculator canonical URL use the production origin.
5. Compare several short and long date ranges against the provider's raw prices and dividend events before requesting search indexing.
