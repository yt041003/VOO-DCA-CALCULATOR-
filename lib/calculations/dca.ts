...
export function calculateDca(input:DcaInput, prices:PricePoint[], dividends:Dividend[]):DcaResult {
  ...
  let shares=0, contributions=0, dividendTotal=0, reinvested=0, cash=0, divIndex=0; const timeline:TimelinePoint[]=[]; let lastMonth='';
  for(let i=0;i<p.length;i++){
    ...
    if(amount){ shares+=amount/price.close; contributions+=amount; }
  ...
  const endingValue=final.portfolioValue, gain=endingValue-contributions, days=Math.max(1,(day(input.endDate).getTime()-day(input.startDate).getTime())/86400000); const cagr=contributions>0&&days>=365?Math.pow(endingValue/contributions,365.25/days)-1:null;
  return {endingValue,investedValue:shares*p.at(-1)!.close,cashDividends:cash,totalContributions:contributions,investmentGain:gain,totalReturn:contributions?gain/contributions:0,shares,dividendsReceived:dividendTotal,dividendsReinvested:reinvested,annualizedReturn:cagr,contributionMultiple:contributions?endingValue/contributions:0,timeline,years};
}
